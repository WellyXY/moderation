...
type TrendingFeedsProps = {
  currentUserId?: string;
};

export function TrendingFeeds({ currentUserId }: TrendingFeedsProps) {
  // 用户ID输入状态
      ...
      .sort((a, b) => {
        ...
        const calculateTrendingScore = (post: Post) => {
          ...
          const boostHours = post.boostedAt
            ? (Date.now() - post.boostedAt.getTime()) / (1000 * 60 * 60)
            : 48;
          ...
          return (
            engagementScore * timeDecayFactor + thresholdBonus + trendingBonus
          );
        };
                ...
                {filteredAndSortedPosts.map((post, index) => {
                  ...
                  return (
                    <tr key={post.id} className="hover:bg-gray-50">
                      ...
                      <td className="border border-gray-200 px-2 py-1">
                        {post.boostedAt?.toLocaleString().split(",")[0] ||
                          "N/A"}
                      </td>
                      <td className="border border-gray-200 px-2 py-1">
                    ...
                    {(() => {
                      ...
                      const thresholdBonus =
                        selectedPost.likes > appliedFeedParams.likeThreshold
                          ? 500
                          : 0;
                      const boostHours = selectedPost.boostedAt
                        ? (Date.now() - selectedPost.boostedAt.getTime()) /
                          (1000 * 60 * 60)
                        : 48;
                      const trendingBonus = selectedPost.isBoosted
                        ? Math.max(1000 - boostHours * 10, 200)
                        : 0;
                      const totalScore =
                        engagementScore * timeDecayFactor +
