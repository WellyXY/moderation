...
const TrendingFeeds: React.FC<TrendingFeedsProps> = ({ currentUserId }) => {
	...
	const mockFeedPosts: Post[] = [
		...
		},
		{
			...
			videoPrompt:
				"Create a mesmerizing digital art speed painting video",
		},
		{
			id: "feed_5",
			userId: "user_005",
		...
		},
		{
			...
			videoPrompt:
				"Create an educational video about cryptocurrency basics",
		},
		{
			id: "feed_16",
			userId: "user_016",
		...
		return mockFeedPosts
			...
			.sort((a, b) => {
				...
				const calculateTrendingScore = (post: Post) => {
					...
					const boostHours = post.boostedAt
						? (Date.now() - post.boostedAt.getTime()) /
							(1000 * 60 * 60)
						: 48
					const trendingBonus = post.isBoosted
						? Math.max(1000 - boostHours * 10, 200)
						: 0
		...
		<div className="p-6">
			...
			<div className="mb-6">
				...
				<p className="text-gray-600">
					Featured and Good content ranked by algorithm scoring
				</p>
			</div>
				...
				{appliedUserId && appliedUserId !== currentUserId && (
					<div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
						<p className="text-sm text-green-700">
							<span className="font-medium">Applied User:</span>{" "}
							{appliedUserId}
						</p>
					</div>
				)}
				...
				<div className="mb-4 p-3 bg-gray-50 rounded-lg">
					<p className="text-sm text-gray-700 font-mono">
						...
						<span className="font-bold">
							(a×like + b×comment + c×remix + d×watch%) ×
							time_decay + (like_count &gt; T) + trending_bonus
						</span>
					</p>
				</div>
			...
			<div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
				<div className="flex justify-between items-center mb-4">
					<h3 className="text-lg font-semibold text-gray-900">
						Trending Content ({filteredAndSortedPosts.length})
					</h3>
						...
						<table className="w-full border-collapse border border-gray-200 text-xs">
							...
							<tbody>
								{filteredAndSortedPosts.map((post, index) => {
									...
									// 使用与排序相同的算法计算完整评分
									const engagementScore =
										appliedFeedParams.likeWeight *
											post.likes +
										appliedFeedParams.commentWeight *
											post.comments +
										appliedFeedParams.remixWeight *
											post.remixes +
										appliedFeedParams.watchWeight *
											post.watchPercentage
									const thresholdBonus =
										post.likes >
										appliedFeedParams.likeThreshold
											? 500
											: 0
									const boostHours = post.boostedAt
										? (Date.now() -
												post.boostedAt.getTime()) /
											(1000 * 60 * 60)
										: 48
									const trendingBonus = post.isBoosted
										? Math.max(1000 - boostHours * 10, 200)
								...
								<div>
									...
									<div className="grid grid-cols-2 gap-4 text-sm">
										{(() => {
											...
											const hoursSince =
												(Date.now() -
													selectedPost.createdAt.getTime()) /
												(1000 * 60 * 60)
											const timeDecayFactor = Math.pow(
												appliedFeedParams.timeDecay,
												hoursSince / 24,
											...
											const thresholdBonus =
												...
												appliedFeedParams.likeThreshold
													...
													: 0
											const boostHours =
												selectedPost.boostedAt
													? (Date.now() -
															selectedPost.boostedAt.getTime()) /
														(1000 * 60 * 60)
													: 48
											const trendingBonus =
												selectedPost.isBoosted
													? Math.max(
															1000 -
																boostHours * 10,
															200,
											...
											return (
												<>
													...
													<div>
														Decay Factor:{" "}
														{timeDecayFactor.toFixed(
															3,
														)}
													</div>
													<div>
														Engagement Score:{" "}
														{engagementScore.toFixed(
															2,
														)}
													</div>
													<div>
